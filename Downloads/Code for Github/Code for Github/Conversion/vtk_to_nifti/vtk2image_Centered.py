...
def convert(poly, rImage):
	...
	"""
	rOrigin = np.array(rImage.GetOrigin())
    
    
	bounds = np.array(poly.GetBounds())
	spacing = np.zeros(3) #desired volume spacing
	spacing[0] = 1
	spacing[1] = 1
	spacing[2] = 1
	origin = np.zeros(3)
	bounds[0] = rOrigin[0]-spacing[0]/2
	bounds[1] = rOrigin[0]+spacing[0]/2
	bounds[2] = rOrigin[1]-spacing[1]/2
	bounds[3] = rOrigin[1]+spacing[1]/2
	bounds[4] = rOrigin[2]-spacing[2]/2
	bounds[5] = rOrigin[2]+spacing[2]/2
